...
mod tests {
    use crate::args::{
        ...
        FunctionArg, FunctionGroup, IdArg, ImAddress, ImArg, ImFunctionType, InArg, LissyIrReport,
        LopcArg, MultiSenseArg, PStat, Pcmd, ProgrammingAbortedArg, PxctData, RFID5Report,
        RFID7Report, RepStructure, SensorLevel, SlotArg, SnArg, SndArg, SourceType, SpeedArg,
    ...
    fn messages() {
        ...
        test_one_message(Message::SwAck(SwitchArg::new(
            2047,
            SwitchDirection::Curved,
            true,
        )));
        test_one_message(Message::SwState(SwitchArg::new(
            0,
        ...
        test_one_message(Message::ConsistFunc(
            SlotArg::new(10),
            DirfArg::new(true, false, false, true, false, false),
        ));
        test_one_message(Message::SlotStat1(
            SlotArg::new(10),
            Stat1Arg::new(true, Consist::Free, State::Idle, DecoderType::Regular28),
        ));
        test_one_message(Message::LongAck(LopcArg::new(10), Ack1Arg::new(true)));
        test_one_message(Message::InputRep(InArg::new(
            10,
            SourceType::Ds54Aux,
            SensorLevel::Low,
            true,
        ...
        test_one_message(Message::SwReq(SwitchArg::new(
            10,
            SwitchDirection::Curved,
            false,
        )));
        test_one_message(Message::LocoSnd(
            SlotArg::new(24),
            SndArg::new(false, true, false, true),
        ));
        test_one_message(Message::LocoDirf(
            SlotArg::new(10),
            DirfArg::new(false, true, false, true, false, false),
        ));
        test_one_message(LocoSpd(SlotArg::new(10), SpeedArg::Drive(122)));

        // Test 6 bytes messages
        test_one_message(Message::MultiSense(
            MultiSenseArg::new(3, false, 3, 7),
            AddressArg::new(12),
        ));
        test_one_message(Message::UhliFun(
            SlotArg::new(128),
            FunctionArg::new(FunctionGroup::F13TO19),
        ));

        // Test messages of variable byte length
        test_one_message(Message::WrSlData(WrSlDataStructure::DataGeneral(
            SlotArg::new(12),
        ...
        test_one_message(Message::WrSlData(WrSlDataStructure::DataPt(
            Pcmd::new(false, true, false, false, false),
            AddressArg::new(64),
            TrkArg::new(true, true, true, true),
            *CvDataArg::new().set_data(1, true).set_cv(2, true),
        )));
        test_one_message(Message::WrSlData(WrSlDataStructure::DataTime(
        ...
        test_one_message(Message::ProgrammingAborted(ProgrammingAbortedArg::new(
            0x15,
            &[
        ...
        test_one_message(Message::Rep(RepStructure::LissyIrReport(
            LissyIrReport::new(true, 77, 66),
        )));
        test_one_message(Message::Rep(RepStructure::WheelcntReport(
            WheelcntReport::new(23, true, 12),
        )));
    ...
    fn test_one_message(message: Message) {
        assert_eq!(
            Message::parse(message.to_message().as_slice()).unwrap(),
            message
        );
    }
    ...
    async fn test_reading() {
        ...
        loop {
            ...
            let opc = port.read_exact(&mut buf).await;
            let byte = match opc {
                Ok(_) => buf[0],
                Err(_) => {
                    return;
    ...
    async fn test_message_sending() {
        ...
        let _m = Message::parse(
            Message::SwReq(SwitchArg::new(15, SwitchDirection::Curved, true))
                .to_message()
                .as_slice(),
        )
        .unwrap();
        ...
        println!(
            "Message: {:?}",
            Message::SwReq(SwitchArg::new(15, SwitchDirection::Curved, true)).to_message()
        );
        ...
        loop {
            match receiver.recv().await {
                Ok(message) => match message {
                    LocoDriveMessage::Message(message) => match message {
        ...
        for i in 1..3 {
            ...
            if i % 2 == 0 {
                loco_controller
                    .send_message(Message::SwReq(SwitchArg::new(
                        15,
                        SwitchDirection::Straight,
                        true,
                    )))
            ...
            loco_controller
                .send_message(LocoSpd(
                    *slot_adr_map.get(&adr).unwrap(),
                    SpeedArg::Drive(100),
                ))
                .await
                .unwrap();
                ...
                match message {
                    LocoDriveMessage::Message(message) => match message {
                        Message::InputRep(in_arg) => {
                            if i % 2 == 0
                                && in_arg.address() == 3
                                && in_arg.input_source() == SourceType::Switch
                                && in_arg.sensor_level() == SensorLevel::High
                            {
                                waiting = false;
                                loco_controller
                                    .send_message(LocoSpd(
                                ...
                                && in_arg.sensor_level() == SensorLevel::High
                            {
                                ...
                                && in_arg.address() == 8
                                && in_arg.input_source() == SourceType::Ds54Aux
                                && in_arg.sensor_level() == SensorLevel::Low
                            {
                                break;
                            } else if !waiting
                                && in_arg.address() == 1
            ...
            loco_controller
                .send_message(LocoSpd(*slot_adr_map.get(&adr).unwrap(), SpeedArg::Stop))
                .await
                .unwrap();
